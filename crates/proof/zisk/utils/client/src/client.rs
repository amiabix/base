...
/// The default interval at which intermediate output roots are recorded.
    ...
    P: Pipeline + SignalReceiver + Send + Sync + Debug,
{
    ...
    loop {
        ...
        if let Some(target_block) = target
            && tip_cursor.l2_safe_head.block_info.number >= target_block
        {
        ...
        let mut attributes = match driver.pipeline.produce_payload(tip_cursor.l2_safe_head).await {
            ...
            Err(PipelineErrorKind::Critical(PipelineError::EndOfSource)) => {
                warn!(
                    target: "client",
                    "exhausted data source; halting derivation and using current safe head"
                );
            ...
            Err(error) => {
                error!(target: "client", ?error, "failed to produce payload");
                return Err(DriverError::Pipeline(error));
            }
        };
        #[cfg(target_os = "zkvm")]
        ...
        let outcome = match driver.executor.execute_payload(attributes.clone()).await {
            ...
            Err(error) => {
                ...
                if !E::is_deposit_only_retryable(&error) {
                ...
                attributes.transactions =
                    attributes.transactions.map(|transactions: Vec<Bytes>| {
                        transactions
                            .into_iter()
                            .filter(|transaction| {
                                !transaction.is_empty() && transaction[0] == OpTxType::Deposit as u8
                            })
                            .collect::<Vec<_>>()
                    });
                ...
                match driver.executor.execute_payload(attributes.clone()).await {
                    ...
                    Err(error) => {
                        error!(
                            target: "client",
                            error = %error,
                            "critical failure executing deposit-only block",
                        );
                        return Err(DriverError::Executor(error));
                    }
                }
            ...
            body: BlockBody {
                transactions: attributes
                    ...
                    .map(|transaction: &Bytes| {
                        BaseTxEnvelope::decode(&mut transaction.as_ref()).map_err(DriverError::Rlp)
                    })
